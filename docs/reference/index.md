...
# equal. Setting this to "one-of" splits the container's label value on
# BACKUP_LABEL_MATCH_SEPARATOR, so a single container labeled
# `stop-during-backup=service1,service2` can be stopped by multiple instances of
# this image, each configured with a different value.
# BACKUP_LABEL_MATCH_BEHAVIOR="match"

# The separator used to split a container's label value when
# BACKUP_LABEL_MATCH_BEHAVIOR is "one-of". Defaults to a comma. Set it to a
# different character if your label values themselves contain a comma.
# BACKUP_LABEL_MATCH_SEPARATOR=","
...
# by `shoutrrr`: https://shoutrrr.nickfedor.com/v0.19.0/services/overview/
