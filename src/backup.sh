...
if [ ! -z "$AWS_S3_BUCKET_NAME" ]; then
  ...
  mc cp $MC_EXTRA_FLAGS "$BACKUP_FILENAME" "backup-target/$AWS_S3_BUCKET_NAME"
...
if [ ! -z "$BACKUP_RETENTION_DAYS" ]; then
  ...
  rule_applies_to=$(mc rm $MC_EXTRA_FLAGS --fake --recursive -force --older-than "${BACKUP_RETENTION_DAYS}d" "backup-target/$bucket" | wc -l)
  ...
  total=$(mc ls $MC_EXTRA_FLAGS "backup-target/$bucket" | wc -l)
  ...
  mc rm $MC_EXTRA_FLAGS --recursive -force --older-than "${BACKUP_RETENTION_DAYS}d" "backup-target/$bucket"
