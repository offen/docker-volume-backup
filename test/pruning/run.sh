...
echo "## Create backup with no prune for s3 backend"
docker compose exec backup backup

echo "## Check if old backup has been pruned (local)"
test ! -f ./local/test-hostnametoken-old.tar.gz
echo "## Check if old backup has NOT been pruned (s3)"
docker run --rm \
  -v minio_backup_data:/minio_data \
  alpine \
...
echo "## Create backup with no prune for both backends"
docker compose exec -e BACKUP_SKIP_BACKENDS_FROM_PRUNE="s3,local" backup backup

echo "## Check if old backup has NOT been pruned (local)"
test -f ./local/test-hostnametoken-old.tar.gz
echo "## Check if old backup has NOT been pruned (s3)"
docker run --rm \
  -v minio_backup_data:/minio_data \
